...
namespace FCG
{
    public class CityStreet
    {
        ...
        public bool isHorizontal; //是否水平方向
        
        public CityStreet(){}
        
        /// <param name="pos">路位置</param>
        /// <param name="towards">朝向</param>
        public CityStreet(Vector3 pos, int towards)
        {
            position = pos;
            rotation = towards < 1 ? Quaternion.identity : Quaternion.Euler(0, 90, 0);
            isHorizontal = towards < 1;
        }
    }
    ...
    public class CityGenerator : MonoBehaviour
    {
        ...
        public Action OnGenerateCityComplete;

        private float m_minZ;
        private float m_maxZ;

        public Dictionary<Vector3, CityStreet> m_cityStreetMap = new Dictionary<Vector3, CityStreet>();
        public Dictionary<Vector3, GameObject> m_cityBuildingMap = new Dictionary<Vector3, GameObject>();
        public void ClearCity()
        {
            m_cityStreetMap.Clear();
            m_cityBuildingMap.Clear();
        ...
        public void GenerateRoad(float mapWidth, float mapHeight)
        {
            ClearCity();
            cityMaker = new GameObject("City-Maker");

            var beginIsHorizontal = false;
            var towards = Random.Range(0f, 1f) > 0.3f ? 0 : 1;  //0 横向街道 1 纵向街道
            //生成下半部分地图数据
            while (m_minZ > -mapHeight * 0.5f)
            {
                var posX = 0f;
                var index = 0;
                ...
                while (posX < mapWidth * 0.5f)
                {
                    if (towards == 0)
                    {
                        stepX = index == 0 ? 0 : 600f;
                    }
                    else
                    {
                        stepX = index == 0 ? 150f : 300;
                    }

                    if (posX == 0 && m_minZ == 0)
                    {
                        beginIsHorizontal = towards < 1;
                    }
                    
                    if (index == 0 && towards == 0)
                    {
                        var initStreet = new CityStreet(new Vector3(0, 0, m_minZ), towards);
                        m_cityStreetMap.Add(initStreet.position, initStreet);
                    }
                    else
                    {
                        var lefRoad = new CityStreet(new Vector3(-(posX + stepX), 0, m_minZ), towards);
                        var rightRoad = new CityStreet(new Vector3( posX + stepX, 0, m_minZ), towards);
                        m_cityStreetMap.Add(lefRoad.position, lefRoad);
                        m_cityStreetMap.Add(rightRoad.position, rightRoad);   
                    }
                    
                    index++;
                    posX += stepX;
                    if (posX >= mapWidth * 0.5f)
                    {
                        var lastTowards = towards;
                        towards = Random.Range(0f, 1f) > 0.3f ? 0 : 1;
                        if (lastTowards == 0)
                        {
                            stepZ = towards < 1 ? 300f : 450f;
                        }
                        else
                        {
                            stepZ = towards < 1 ? 450f : 600f;
                        }
                        m_minZ -= stepZ;
                        break;
                    }
            ...
            if (towards == 0)
            {
                m_maxZ = beginIsHorizontal ? 300f : 450f;
            }
            else
            {
                m_maxZ = beginIsHorizontal ? 450f : 600f;
            }
            
            //生成上半部分地图数据
            while (m_maxZ < mapHeight * 0.5f)
            {
                var posX = 0f;
                var index = 0f;
                ...
                while (posX < mapWidth * 0.5f)
                {
                    ...
                    {
                        stepX = index == 0 ? 150f : 300f;
                    }

                    if (index == 0 && towards == 0)
                    {
                        var initStreet = new CityStreet(new Vector3(0, 0, m_maxZ), towards);
                        m_cityStreetMap.Add(initStreet.position, initStreet);
                    }
                    else
                    {
                        var leftStreet = new CityStreet(new Vector3(-(stepX + posX), 0, m_maxZ), towards);
                        var rightStreet = new CityStreet(new Vector3(stepX + posX, 0, m_maxZ), towards);
                        m_cityStreetMap.Add(leftStreet.position, leftStreet);
                        m_cityStreetMap.Add(rightStreet.position, rightStreet);   
                    }
            ...
            //创建路
            foreach (var kv in m_cityStreetMap)
            {
                var road = Instantiate(largeBlocks[Random.Range(0, largeBlocks.Length)], kv.Value.position, kv.Value.rotation, cityMaker.transform);
                
                //合并路边装饰物
                var objects = road.transform.Find("Objects");
                if (objects != null)
                {
                    //MeshUtility.CombineMeshes(objects, true);   
                }
                
                //合并路灯mesh
                var trafficlights = road.transform.Find("TrafficLights");
                if (trafficlights != null)
                {
                    //'MeshUtility.CombineMeshes(trafficlights, true);   
                }
            }
        ...
        public void GenerateAllBuildings(bool _withDowntownArea, float _downTownSize)
        {
            ...
            Debug.Log($"Building Center: {center}");
            ...
            foreach (var kv in m_cityBuildingMap)
            {
                var decoration = kv.Value.transform.Find("Decoration");
                if (decoration != null)
                {
                    MeshUtility.CombineMeshes(decoration, true);   
                }
            }
            
            OnGenerateCityComplete?.Invoke();
            DestroyImmediate(pB);
            DayNight dayNight = FindObjectOfType<DayNight>();
            if(dayNight)
            {
        ...
        public bool CreateBuildingsInCorners(GameObject child, bool notAnyone = false)
        {
            ...
            AddBuilding(pBuilding.transform.position, pBuilding);
            nB++;
            ...
            foreach (GameObject bks in tempArray)
            {
                ...
                foreach (Transform bk in bks.transform)
                {
                    ...
                    if (Random.Range(0, 20) > 5)
                    {
                        ...
                        var building = Instantiate(BK[numB], bk.position, bk.rotation, bk);
                        AddBuilding(bk.position, building);
                        nB++;
        ...
        public void CreateBuildingsInSuperBlocks()
        {
            ...
            foreach (GameObject bks in tempArray)
            {
                foreach (Transform bk in bks.transform)
                {
                    ...
                    var building = Instantiate(SB[numB], bk.position, bk.rotation, bk);
                    AddBuilding(bk.position, building);
                    nB++;
                }
            }
        ...
        private void CreateBuildingsInDoubleLine(GameObject line)
        {
            ...
            while (tt < 100)
            {
                ...
                else
                {
                    ...
                    {
                        AddBuilding(pBuilding[index].transform.position, pBuilding[index]);
                    }
        ...
        private void AddBuilding(Vector3 position, GameObject building)
        {
            if (!m_cityBuildingMap.TryAdd(position, building))
            {
                Debug.Log($"AddBuilding Has Same Key: Position: {position} Building: {building.name}");
            }
        }
        ...
        private void AdjustsWidth(GameObject[] tBuildings, int quantity, float remainingMeters, float init, bool slope = false)
        {
            ...
            for (int i = 0; i < quantity; i++)
            {
                ...
                AddBuilding(tBuildings[i].transform.position, tBuildings[i]);
            }
        ...
        private float GetWith(GameObject building)
        {
            ...
            {
                var house = building.transform.Find("House").GetChild(0);
                return house.GetComponent<MeshFilter>().sharedMesh.bounds.size.z;
            }
        }
