...
namespace Runtime.AdvancedBundleSystem.Common.Util
{
    ...
    public struct PooledBuffer : IDisposable
    {
        ...
        {
            if (this.Data != null)
                ABSBufferPool.Release(this.Data);
            this.Data = null;
        }
    }
    ...
    public static class ABSBufferPool
    {
        ...
        public static byte[] Resize(ref byte[] buffer, int newSize, bool canBeLarger)
        {
            ...
            byte[] newBuf = ABSBufferPool.Get(newSize, canBeLarger);
            Array.Copy(buffer, 0, newBuf, 0, Math.Min(newBuf.Length, buffer.Length));
            ABSBufferPool.Release(buffer);
                ...
                if (store.Size == bufferLength)
                {
                    ...
                    if (IsDoubleReleaseCheckEnabled)
                        for (int cv = 0; cv < store.buffers.Count; ++cv)
                        {
                            ...
                            {
                                LoggerInternal.LogError("VariableSizedBufferPool, Buffer already added to the pool!");
                                return;
                            }
                        }
